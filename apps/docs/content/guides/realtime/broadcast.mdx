...
Broadcast Changes allows you to trigger messages from your database. To achieve it we're directly reading your WAL file using a publication against the `realtime.messages` table so whenever a new insert happens we are able to broadcast it to your users.
...
Realtime Authorization is required and enabled by default. To allow your users to listen to messages from topics, create a RLS policy:
